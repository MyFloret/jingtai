// 获取用户IP位置
$.ajax({
    url: "https://api.9918.tv/Sites/GetUserArea",
    type: "get",
    success: function (res) {
        var data = JSON.parse(res);
        $("#province").val(data.province)
        assignment(data.province, 1)
        $("#city").val(data.city)
        assignment(data.city, 2)
        $("#town").val(data.district)
    }
})
var provinceList = [
    {
...
function assignment(value, num) {
    if (num == 1) {
        provinceText = value;
        $.each(provinceList, function (i, item) {
            if (item.name.slice(0, 2) == provinceText || item.name == provinceText) {
                cityItem = i;
                return cityItem
            }
        });
