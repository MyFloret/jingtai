	...
	<view>
		...
		<scroll-view v-else scroll-y="true" :style="{height: scrollHeight}">
			<view v-for="(item, index) in list" :key="index">
				<view v-if="item.ProcPic && item.ProcTitle" class="myCard2">
					<view class="leftImg">
						<u-image height="160" width="160" border-radius="26" :src="item.ProcPic"></u-image>
					</view>
					<view class="rightView">
						<view class="cardTopName1">{{item.ProcTitle}}</view>
						<view class="rowBtn">
		...
		methods: {
			...
			searchBoxFun: function() {
				...
				const subArr = {
					...
					Id: this.productId,
					ProcTitle: that.searchValue1,
					ProcPic: this.productSrc,
					DealerId: this.DealerId,
					Remark: ""
				}
				this.$http.post('Dealer/SaveProductForm', subArr).then(res => {
					if (res.Tag == 1) {
			...
			getList(e) {
				this.$http.get(`Dealer/GetProductPageList`, {
					DealerId: e,
					Token: uni.getStorageSync('WebToken_CRM')
				}).then(res => {
				...
				this.$http.get(`Dealer/GetImagesPageList`, {
					...
					res.Data.forEach((item)=>{
						console.log(item)
						ImgArr.push({
							ProcTitle: item.ImagesName ,
							ProcPic: item.ImagesPath
						})
					})
				})
			},
			...
			deleteCpFun: function(item, index) {
				...
				uni.showModal({
					title: '提示',
					content: '是否确认删除产品：' + item.ProcTitle,
					success(rrr) {
						if (rrr.confirm) {
							uni.showLoading({
								title: '删除中...'
							})
							that.$http.post(`File/DeleteFile?fileModule=5&filePath=${item.ProcPic.split('/Resource/Dealer')[1]}`).then(
								res => { 
									if (res.Tag == 1) {
										uni.showToast({
											title: res.Message,
							...
							that.$http.post(`Dealer/DeleteProductFormJson?ids=${item.Id}`).then(
								res => { 
									if (res.Tag == 1) {
										uni.showToast({
											title: res.Message,
