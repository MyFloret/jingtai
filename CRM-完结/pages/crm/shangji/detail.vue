...
<template>
	<view class="content">
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false" style="height: calc(100% - 55px);" @scrolltolower="scrollToBottomFun">
			<!--客户基本信息-->
			<shangjiCard :item="infoObj" :isDetail="true"></shangjiCard>
			<!--客户阶段-->
			<view class="myCard">
				<view class="cardTopName disFlexJ">
					<text>商机阶段</text>
					<view style="color: #F29100;" @click="selectShow = true">
						更改
						<u-icon name="edit-pen-fill" color="#F29100"></u-icon>
					</view>
				</view>
				<u-steps class="width100" mode="number" :list="jieDuanList" :current="jieDuanIndex"></u-steps>
			</view>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<sjDetail :infoObj="infoObj" :isQywx="isQywx"></sjDetail>
				</view>
				<!--产品信息-->
				...
				<view v-if="tabIndex == 3">
					<view v-for="(item, index) in tabList[3].arr" :key="index">
						<baojiadan :item="item" :isQywx="isQywx"></baojiadan>
					</view>
				</view>
				<!--合同-->
				<view v-if="tabIndex == 4">
					<view v-for="(item, index) in tabList[4].arr" :key="index">
						<hetong :item="item" :isQywx="isQywx"></hetong>
					</view>
				</view>
				<!--操作记录-->
				<view v-if="tabIndex == 5">
					<view>
						<czjl :infoObj="infoObj" :dataId="infoObj._id" fl="商机" :isQywx="isQywx" pageType="business"></czjl>
					</view>
				</view>
				<!--加载更多-->
...
<script>
	...
	import shangjiCard from '@/components/card/shangji.vue'
	import cp1Card from '@/components/card/cp1Card.vue'
	import sjDetail from '@/components/detail/sjDetail.vue'
	...
	export default {
		...
		data() {
			return {
				...
				tabList: [ 
					...
					{ name: '产品信息', arr: [], isMore: true },
					{ name: '跟进记录', arr: [], pageIndex: 1, isMore: true },
					{ name: '报价单', arr: [], pageIndex: 1, isMore: true },
					{ name: '合同', arr: [], pageIndex: 1, isMore: true },
		...
		onBackPress(e) {
			if(e.from == 'backbutton' && this.cardIndex != -1) {
				...
				uni.$emit('updateSjListByIndex', { index: this.cardIndex, obj})
			}
		},
		methods: {
			tabChange(index) {
			...
			getGenJinFun: function() {
				...
				if(that.isQywx) {
					reqData.params.matchObj.external_userid = that.infoObj.external_userid;
				} else {
					reqData.params.matchObj.clientId = that.infoObj.clientId;
				}
			...
			pageGenjinFun: function() {
				...
				uni.navigateTo({
					url: '../genJin/add?clientName=' + that.infoObj.clientName + '&clientId=' + that.infoObj.clientId + '&glkhLxr=' + glkhLxr + '&glkhLxrId=' + that.infoObj.clientLxrId + '&orderType=商机' + '&glOrderId=' + that.infoObj._id + '&glOrder=' + that.infoObj.sjName + '&isQywx=' + that.isQywx +'&external_userid=' + external_userid
				})
			},
			// 修改信息
			pageBianjiFun: function() {
				...
				uni.navigateTo({
					url: './addSj?type=update&isQywx=' + that.isQywx
				})
			},
			// 更多回调方法
			pageDeleteShowFun: function(e) {
				uni.showModal({
					title: '提示',
					content: '是否确认商机名称为：' + that.infoObj.sjName + '的商机吗?',
					success(res) {
						if(res.confirm) {
							uni.showLoading({
								title: '删除中...',
			...
			selectConfirmFun: function (e) {
				...
				crmShangJiApi(reqData)
				.then(res => {
					that.infoObj.sjJieduan = str;
					that.infoObj.percent = percent;
					...
					let czjlObj = {
						...
						czRen: uni.$userInfo._id,
						pageType: 'business',
						dataId: that.infoObj._id,
						type: '编辑',
						content: [ '修改 商机阶段 ，由 "' + that.infoObj.sjJieduan + '" 变更为 "' + str + '"' ]
					}
					uni.$czjlApiAddFun(czjlObj);
				})
			},
