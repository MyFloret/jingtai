<template>
	<view>
		<topDropdown :isSxShow="true" :isPaiXu="true" :dropdown1="dropdown1" :options1="options1" :optionsSx="sxList" :optionsPx="optionsPx" :isDuoXuan="true" searchLabel1="客户名称" searchPh1="请输入客户名称" @searchBoxEmitFun="searchBoxEmitFun" @dropDownFun1="dropDownFun1" @optionSxFun="optionSxFun" @optionPxFun="optionPxFun"></topDropdown>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectKehuFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index">
					<threadCard :item="item" :isSelect="isSelect" :index="index" @cxGetDataFun="cxGetDataFun"></threadCard>
				</view>
		...
		<addBtn url="./addThread?isAdd=1"></addBtn>
...
<script>
	...
	import { tyDropDown, dropDownByUser, threadSxData, datePxData } from '../../../static/utils/dropdown.js'
	import {
	...
	import threadCard from '../../../components/card/threadCard.vue'
	export default {
		components: {
			dataNull,
			addBtn,
		...
		onLoad(e) {
			that = this;
			let threadSxList = JSON.stringify(threadSxData);
			that.sxList = JSON.parse(threadSxList);
			let obj = {};
			...
			uni.$on('threadListChangeFun', that.cxGetDataFun)
		},
		onBackPress() {
			...
			uni.$off('threadListChangeFun', that.cxGetDataFun)
		},
		methods: {
			// 查询客户
