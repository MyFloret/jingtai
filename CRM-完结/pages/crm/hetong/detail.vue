			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 3">
					<view v-for="(item, index) in tabList[3].arr" :key="index">
						<huikuanJH :item="item" :isQywx="isQywx"></huikuanJH>
					</view>
				</view>
				<!--跟进记录-->
				<view v-if="tabIndex == 4">
					<view v-for="(item, index) in tabList[4].arr" :key="index">
						<shijiHK :item="item" :isQywx="isQywx"></shijiHK>
					</view>
				</view>
				<!--操作记录-->
				<view v-if="tabIndex == 5">
					<view>
						<czjl :infoObj="infoObj" fl="合同" :dataId="infoObj._id" pageType="contract"></czjl>
					</view>
				</view>
				<!--加载更多-->
	...
	export default {
		...
		data() {
			return {
				...
				tabList: [ 
					...
					{ name: '回款单', arr: [], pageIndex: 1, isMore: true },
		...
		methods: {
			...
			pageGenjinFun: function() {
				...
				uni.navigateTo({
					url: '../genJin/add?clientName=' + that.infoObj.clientName + '&clientId=' + that.infoObj.clientId + '&glkhLxr=' + glkhLxr + '&glkhLxrId=' + that.infoObj.clientLxrId + '&orderType=合同订单' + '&glOrderId=' + that.infoObj._id + '&glOrder=' + that.infoObj.htCode + '&isQywx=' + that.isQywx + '&external_userid=' + external_userid
				})
			},
			// 修改信息
