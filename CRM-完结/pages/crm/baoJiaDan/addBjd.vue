<template>
	<view>
		...
		<view class="flex-white-plr26 ptb10 bdb_f5 mr16">
			<text class="mr26">报价金额
				<text class="redXingh">*</text>
			</text>
			<u-input v-model="bjPrice" input-align="right" placeholder="请在此输入或添加产品自动计算" @input="orderPriceXgFun" />
		</view>
		<view class="flex-white-plr26 ptb20 bdb_f5">
			<text class="mr26">报价时间<text class="redXingh">*</text></text>
			<picker mode="date" :value="bjdDate" start="1999-02-15" end="2100-02-15" @change="bindDateChange">
			    <view v-if="bjdDate" class="w420">{{bjdDate}}</view>
			    <view v-else style="color: #c5c5c5;">请选择报价日期</view>
			</picker>
		...
		methods: {
			...
			selectKehuFun: function() {
				if(!that.clientDisabled) {
					if(!that.isQywx) {
						uni.navigateTo({
							url: '../kehu/kehu?type=bjd'
						})
					} else {
						uni.navigateTo({
							url: '../../../qywxPage/view/qywxClient/qywxClient?type=bjd'
						})
					}
				}
			},
			...
			selectShangJiFun: function() {
				...
				if(that.isQywx) {
					uni.navigateTo({
						url: '../shangji/shangji?type=bjd&isQywx=true&external_userid=' + that.external_userid
					})
				} else {
					uni.navigateTo({
