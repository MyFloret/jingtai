...
$.each(provinceList, function (i, item) {
    addEle(provinceEle, item.name);
});
provinceEle.on('click', 'li', function () {
    console.log($(this))
    provinceEle.find('.active').removeClass('active');
    $(this).addClass('active');
    selectedProvince = $(this).text();
    var provinceIndex = getIndex(provinceList, selectedProvince);
    var cityList = provinceList[provinceIndex].cityList;
    cityEle.empty();
    areaEle.empty();
    $.each(cityList, function (i, item) {
        addEle(cityEle, item.name);
    });
    quEle.text('请选择');
    shiEle.text('请选择');
    shengEle.text(selectedProvince);
    if (shengEle.text() != "请选择") {
        $('.province').css('display', 'none')
        $('.city').css('display', 'block')
        $('.area').css('display', 'none')
    }
});
cityEle.on('click', 'li', function () {
    cityEle.find('.active').removeClass('active');
    $(this).addClass('active');
    selectedCity = $(this).text();
    var provinceIndex = getIndex(provinceList, selectedProvince);
    var cityIndex = getIndex(provinceList[provinceIndex].cityList, selectedCity);
    var areaList = provinceList[provinceIndex].cityList[cityIndex].areaList;
    areaEle.empty();
    ...
    shiEle.text(selectedCity);
});
areaEle.on('click', 'li', function () {
    selectedArea = $(this).text();
    quEle.text(selectedArea);
});
$('.sheng').click(() => {
...
$(document).on('click', '.city>ul>li', function () {
    if ($(this).hasClass('activediqu')) {
        return;
    }
    $('.city>ul>li.activediqu').removeClass('activediqu');
    $(this).addClass('activediqu');
});
$(document).on('click', '.province>ul>li', function () {
    if ($(this).hasClass('activediqu')) {
        return;
    }
    $('.province>ul>li.activediqu').removeClass('activediqu');
    $(this).addClass('activediqu');
});
$(document).on('click', '.area>ul>li', function () {
    ...
    $('.area>ul>li.activediqu').removeClass('activediqu');
    $(this).addClass('activediqu');
});