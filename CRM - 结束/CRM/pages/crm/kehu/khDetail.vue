<template>
	<view class="content">
		...
		<bottomBox :sheetList="sheetList" :addSheetList="addSheetList" :item="khInfo" :isGhc="khInfo.status == 0" :isKh="khInfo.status == 1" @pageGenjinFun="pageGenjinFun" @pageDeleteShowFun="pageDeleteShowFun" @pageAddSheetFun="pageAddSheetFun" @pageBianjiFun="pageBianjiFun" @upImage="upImage" @xiangxiang="xiangxiang" @cxPGetDataFun="cxPGetDataFun"></bottomBox>
	...
	export default {
		...
		methods: {
			...
			upImage:function(){
				uni.$infoObj = that.khInfo;
				uni.navigateTo({
					url: `../../../chanpin/view/chanpin?id=${this.id}`,
					// ../../chanpin/view/chanpin
				})
			},
			xiangxiang:function(){
				uni.$infoObj = that.khInfo;
				uni.navigateTo({
					url: `../../../chanpin/view/xingxiang?id=${this.id}`,
				})
			},
			// 新增sheet回调方法  新增跳转
