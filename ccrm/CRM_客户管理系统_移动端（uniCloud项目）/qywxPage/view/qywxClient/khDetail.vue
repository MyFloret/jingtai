		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false" style="height: calc(100% - 55px);" @scrolltolower="scrollToBottomFun">
			...
			<!-- <view class="myCard">
				<view class="cardTopName disFlexJ">
					<text>客户阶段</text>
					<view v-if="khInfo.status == 1" style="c
					olor: #F29100;" @click="pickerSelectFun('客户阶段')">
						更改
						<u-icon name="edit-pen-fill" color="#F29100"></u-icon>
					</view>
				</view>
				<u-steps class="width100" mode="number" active-color="#00aa00" :list="jieDuanList" :current="jieDuanIndex"></u-steps>
			</view> -->
			<!-- <view class="myCard">
				<view class="cardTopName disFlexJ">
					<text>客户状态</text>
...
<script>
	...
	import khCard from '@/components/card/qywxkehu.vue'
	import bottomBox from '@/components/bottomBox/bottomBox'
	import khDetail from '@/components/detail/qywxKehuDetail'
	...
	export default {
		...
		data() {
			return {
				...
				tabList: [ 
					...
					// { name: '联系人', arr: [], pageIndex: 1, isMore: true },
					{ name: '商机', arr: [], pageIndex: 1, isMore: true },
					{ name: '报价单', arr: [], pageIndex: 1, isMore: true },
					{ name: '合同', arr: [], pageIndex: 1, isMore: true },
					{ name: '回款计划', arr: [], pageIndex: 1, isMore: true },
					{ name: '实际回款', arr: [], pageIndex: 1, isMore: true },
					// { name: '发票', arr: [], pageIndex: 1, isMore: true },
					// { name: '退货退款', arr: [], pageIndex: 1, isMore: true },
		...
		onLoad(e) {
			...
			if(e.fromDetail) {
				that.selectKhByIdFun(e.external_userid)
			}
		},
		onBackPress(e) {
		...
		methods: {
			...
			getGenJinFun: function() {
				...
				let reqData = {
					...
					params: {
						isQywx: true,
						matchObj: {
							external_userid: that.khInfo.external_userid,
						},
						sortObj: {
							update_date: -1
						},
			...
			pageAddSheetFun: function(e) {
				...
				uni.navigateTo({
					url: url + '?isQywx=true&clientName=' + that.khInfo.remark + '&clientId=' + that.khInfo._id + '&external_userid=' + that.khInfo.external_userid + '&pageT=client'
				})
			},
			// 更多回调方法
			...
			// 根据id查询客户
			selectKhByIdFun: function(external_userid) {
				uni.showLoading({
					title: '加载中...',
					mask: true
				})
				let reqData = {
					action: 'selectKhByExternal_userid',
					params: {
						external_userid: '',
						userid: that.khInfo.userid
					}
				}
				if(external_userid) {
					reqData.params.external_userid = external_userid;
				} else {
					reqData.params.external_userid = that.khInfo.external_userid;
				}
