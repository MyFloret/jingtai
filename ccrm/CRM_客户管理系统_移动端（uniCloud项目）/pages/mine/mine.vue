<template>
	<view>
		<!--个人信息栏-->
		<view class="topInfo">
			<image v-if="userInfo._id" src="../../static/img/logo.png"></image>
			<image v-else src="../../static/img/logo.png"></image>
			<view v-if="userInfo._id">
				<text v-if="userInfo.nickname && userInfo.zhiwei">{{userInfo.zhiwei}}-{{userInfo.nickname}}</text>
				<text v-else-if="userInfo.nickname">{{userInfo.nickname}}</text>
				<text v-else>秒火网CRM体验</text>
				<text>欢迎登陆</text>
			</view> 
		...
		<view class="cardList">
			...
			<navigator v-if="userInfo._id" url="../regLogin/setPassword" class="row">
				<view class="rowLeft">
			...
			<view v-if="userInfo._id" class="row" @click="logoutFun">
				<view class="rowLeft">
						...
						if (res.confirm) {
							...
							crmMyAppUserApi(reqData)
								.then(res => {
									...
									uni.removeStorageSync('qwUser')
									uni.removeStorageSync('classifyList')
									uni.removeStorageSync('qwXsUserList')
									uni.removeStorageSync('qwDepList')
									uni.removeStorageSync('qwXsUserIdList')
