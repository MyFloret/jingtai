<template>
	<view class="content">
		...
		<view v-else class="swiper-item">
			<view v-for="(item, index) in cardList2" :key="index" @click="skipPageFun(item)" class="kongKimCard">
				<view :style="{backgroundImage: item.bgColor}">
					<image :src="item.src" mode="aspectFill"></image>
				</view>
				<view>{{item.name}}</view>
		...
		<qwSalesJianBaoBoard v-if="baoBiaoShow && tabIndex == 1" :startDateTime="startDateTime" :endDateTime="endDateTime"
...
<script>
	...
	export default {
			...
			return {
				...
				cardList2: [{
					...
					},
					{
						...
						src: '/static/icon/genjin.png',
						name: '企微跟进',
						url: '../crm/genJin/genJin?isQywx=true',
						menu_id: 'followUp'
					},
					{
						bgColor: 'linear-gradient(45deg, #62dbff, #ff5aaa)',
						src: '/static/icon/business.png',
						name: '企微商机',
						url: '../crm/shangji/shangji?isQywx=true',
						menu_id: 'business'
					},
					{
						bgColor: 'linear-gradient(45deg, #00d6d6, #a0efef)',
						src: '/static/icon/baojiadan.png',
						name: '报价单',
						url: '../crm/baoJiaDan/baoJiaDan?isQywx=true',
						menu_id: 'quotation'
					},
					{
						bgColor: 'linear-gradient(45deg, #ffaa00, #ffaa7f)',
						src: '/static/icon/contract.png',
						name: '企微合同',
						url: '../crm/hetong/hetong?isQywx=true',
						menu_id: 'contract'
					}
				],
				depList: [],
		...
		onLoad() {
			...
			that.userInfo = uni.$userInfo || {};
			uni.$qwUser = uni.getStorageSync('qwUser') || '';
			that.qwUserInfo = uni.$qwUser || {};
			...
			if (uni.$userInfo.isDepManager) {
				that.getDepartmentFun();
				that.department_id = uni.$userInfo.department_id;
				that.depId = that.department_id[that.department_id.length - 1];
			} else {
				that.fuZeRenId = uni.$userInfo._id;
			}
			let qwConfigEnable = uni.getStorageSync('qwConfigEnable') || false;
			that.tabShow = qwConfigEnable;
			that.qwConfigEnable = qwConfigEnable;
			if (uni.$qwUser && qwConfigEnable) {
				if (uni.$qwUser.isleader) {
					that.getQywxDepartmentFun();
				} else {
