// $.ajax({
//     url: "https://api.spdl.com/Sites/GetUserArea",
//     type: "get",
//     success: function (res) {
//         console.log(res);
//         var data = JSON.parse(res);
//         $("#provinceHeader").val(data.province)
//         $("#provinceProduct").val(data.province)
//         assignmentTwo(data.province, 1)
//         assignmentStree(data.province, 1)
//         $("#cityHeader").val(data.city)
//         $("#cityProduct").val(data.city)
//         assignmentTwo(data.city, 2)
//         assignmentStree(data.city, 2)
//         $("#townHeader").val(data.district)
//         $("#townProduct").val(data.district)
//     }
// })
var provinceListTwo = [
    {
...
var provinceTwo = $("#provinceHeader"), cityTwo = $("#cityHeader"), townTwo = $("#townHeader");
for (var i = 0; i < provinceListTwo.length; i++) {
    addEleTwo(provinceTwo, provinceListTwo[i].name);
}
function addEleTwo(ele, value) {
    var optionStr = "";
    optionStr = "<option value=" + value + ">" + value + "</option>";
    ele.append(optionStr);
}
function removeEleTwo(ele) {
    ele.find("option").remove();
    var optionStar = "<option value=" + "请选择" + ">" + "请选择" + "</option>";
    ele.append(optionStar);
}
var provinceText, cityText, cityItem;
provinceTwo.on("change", function () {
    cityItem = ""
    provinceText = $(this).val();
    $.each(provinceListTwo, function (i, item) {
        if (provinceText == item.name) {
            cityItem = i;
            return cityItem
        }
    });
    removeEleTwo(cityTwo);
    removeEleTwo(townTwo);
    if (cityItem >= 0) {
        $.each(provinceListTwo[cityItem].cityList, function (i, item) {
            addEleTwo(cityTwo, item.name)
        })
    }
});
cityTwo.on("change", function () {
    cityText = $(this).val();
    removeEleTwo(townTwo);
    $.each(provinceListTwo, function (i, item) {
        if (provinceText == item.name) {
    ...
    $.each(provinceListTwo[cityItem].cityList, function (i, item) {
        if (cityText == item.name || item.name.slice(0, 2) == provinceText) {
            for (var n = 0; n < item.areaList.length; n++) {
                addEleTwo(townTwo, item.areaList[n])
            }
        }
    });
});
function assignmentTwo(value, num) {
    if (num == 1) {
        provinceText = value;
        $.each(provinceListTwo, function (i, item) {
            if (provinceText == item.name || item.name.slice(0, 2) == provinceText) {
                cityItem = i;
                return cityItem
            }
        });
        ...
        if (provinceListTwo[cityItem] && provinceListTwo[cityItem].cityList) {
            $.each(provinceListTwo[cityItem].cityList, function (i, item) {
                addEleTwo(cityTwo, item.name);
            });
        }
        ...
        if (provinceListTwo[cityItem] && provinceListTwo[cityItem].cityList) {
            $.each(provinceListTwo[cityItem].cityList, function (i, item) {
                if (cityText === item.name && item.areaList) {
                    for (var n = 0; n < item.areaList.length; n++) {
                        addEleTwo(townTwo, item.areaList[n]);
                    }
                }
...
var provinceStree = $("#provinceProduct"), cityStree = $("#cityProduct"), townStree = $("#townProduct");
for (var i = 0; i < provinceListStree.length; i++) {
    addEleStree(provinceStree, provinceListStree[i].name);
}
function addEleStree(ele, value) {
    var optionStr = "";
    optionStr = "<option value=" + value + ">" + value + "</option>";
    ele.append(optionStr);
}
function removeEleStree(ele) {
    ele.find("option").remove();
    var optionStar = "<option value=" + "请选择" + ">" + "请选择" + "</option>";
    ele.append(optionStar);
}
...
var provinceText, cityText, cityItem;
provinceStree.on("change", function () {
    cityItem = ""
    provinceText = $(this).val();
    $.each(provinceListStree, function (i, item) {
        if (provinceText == item.name) {
            cityItem = i;
            return cityItem
        }
    });
    removeEleStree(cityStree);
    removeEleStree(townStree);
    if (cityItem >= 0) {
        $.each(provinceListStree[cityItem].cityList, function (i, item) {
            addEleStree(cityStree, item.name)
        })
    }
});
cityStree.on("change", function () {
    cityText = $(this).val();
    removeEleStree(townStree);
    $.each(provinceListStree, function (i, item) {
        if (provinceText == item.name) {
    ...
    $.each(provinceListStree[cityItem].cityList, function (i, item) {
        if (cityText == item.name || item.name.slice(0, 2) == provinceText) {
            for (var n = 0; n < item.areaList.length; n++) {
                addEleStree(townStree, item.areaList[n])
            }
        }
    });
});
function assignmentStree(value, num) {
    if (num == 1) {
        provinceText = value;
        $.each(provinceListStree, function (i, item) {
            if (provinceText == item.name || item.name.slice(0, 2) == provinceText) {
        ...
        if (provinceListStree[cityItem] && provinceListStree[cityItem].cityList) {
            $.each(provinceListStree[cityItem].cityList, function (i, item) {
                addEleTwo(cityStree, item.name);
            });
        }
    } else {
        cityText = value;
        ...
        if (provinceListStree[cityItem] && provinceListStree[cityItem].cityList) {
            $.each(provinceListStree[cityItem].cityList, function (i, item) {
                if (cityText === item.name && item.areaList) {
                    for (var n = 0; n < item.areaList.length; n++) {
                        addEleTwo(townStree, item.areaList[n]);
                    }
                }
            });
        }
