<template>
	<view class="_t">
		<view class="tab-bar_t">
			...
			<view class="tab_t" @click="food()">食品</view>
			<view class="tab_t" @click="frink()">饮料</view>
			<view class="tab_t" @click="jiushui()">酒水</view>
			<view class="tab_t" @click="twpa()">调味料</view>
		</view>
		...
		<view class="exhitions_tent">
			<text class="taName">
				第109届全国（深圳）秋季糖酒会布展酒店
				交通指南
			</text>
			<text class="timesa">
				酒店展时间：2023.10.7-10.11
			</text>
			<text class="timesa">
				会展中心展时间：2023.10.12-10.14
			</text>
		</view>
		<!-- 线路图内容 -->
		<view class="line_orgram" v-show="flag">
			...
			<view class="right_content" >
				<view class="exhitions_content" v-for="(item,index) in szqjht">
					<view class="teName_name">{{item.name}}<text class="adress_active">({{item.area}})</text></view>
		...
		methods: {
			...
			getHtml(index = 0) {
				request('/Home/GetSiteConfigData', 'GET', {
					...
					this.szqjht = []
					this.szqjht = JSON.parse(res.data).data[index].jiudian
					console.log(this.szqjht)
				})
			}
		}
	}
