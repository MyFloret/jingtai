	...
	var methods = {
		...
		pageRender: function(_v, _current){
			...
			if(_page>o.pageStep&&_current<=_page){
				...
				if(_current<=_middle){
					_html += methods.forEach(o.url,1, o.pageStep, _current, o.active);
					_html += methods.elliPsis();
				}else if(_current>_middle&&_current<(_page-_tep)){
					_html += methods.pageBtn(o.url,1);
					_html += methods.elliPsis();
					_html += methods.forEach(o.url,_current-_tep, _current-(-_tep)-(-1), _current, o.active);
					_html += methods.elliPsis();
				}else if(_current>=(_page-_tep)){
					_html += methods.pageBtn(o.url,1);
					_html += methods.elliPsis();
					_html += methods.forEach(o.url,_page-2*_tep-1, _page-(-1), _current, o.active);
				}
				...
				_html += methods.forEach(o.url,1, _page-(-1), _current, o.active);
				if(_page>o.minPage){
					_html += methods.setPrevNext(o, 'next');
				}	
			}
		...
		forEach: function(v_c,_start,length,_current,curclass){
			/**
			 * [s page elements]
			 * @type {String}
			 */
		...
		pageBtn: function(v_c,_id){
			 ...
			 */
				 ...
				 let v_cs = v_c.replace("{page}", _id)
      console.log(v_cs);
      if (v_cs.indexOf("-p1.html") !== -1) {
        v_cs = v_cs.replace(/-p1\.html/, "");
        if (!/^\/\w+\/(?=\/$|$)/.test(v_cs)) {
          v_cs = v_cs+'.html'
        }
        console.log(v_cs); // 输出"example"
        return '<a href="' + v_cs + '" page-id="' + _id + '">' + _id + '</a>';
      } else {
        console.log(v_cs); // 输出"example-p1.html"
        return '<a href="' + v_cs + '" page-id="' + _id + '">' + _id + '</a>';
      }
...
// 改动
		...
		addBtn: function(v_c,_property, _page, _count){
			/**
			...
			let v_cs = v_c.replace("{page}", _page)
      console.log(v_cs);
      if (v_cs.indexOf("-p1.html") !== -1) {
        v_cs = v_cs.replace(/-p1\.html/, "");
        if (!/^\/\w+\/(?=\/$|$)/.test(v_cs)) {
        ...
        console.log(v_cs); // 输出"example"
        return '<a href="'+v_cs+'" class="'+_property+'" page-id="'+_page+'" '+disabled+'></a>';
      }else if (v_cs.indexOf("-p0.html") != -1) {
        v_cs = ""
        return '<a href="' + v_cs + '" class="' + _property + '" page-id="' + _page + '" ' + disabled + '></a>';
      } else {
        console.log(v_cs); // 输出"example-p1.html"
		...
		setPrevNext: function(_o, _type){
			...
			function prev(){
				if(_o.btnShow){
					s += methods.addBtn(_o.url,_o.firstBtn, 1); 
				}
				if(_o.btnBool){
					s += methods.addBtn(_o.url,_o.prevBtn, _o.current-1, _o.pageCount);
				}
				return s;
			}
			function next(){
				if(_o.btnBool){
					s += methods.addBtn(_o.url,_o.nextBtn, _o.current-(-1), _o.pageCount);
				}
				if(_o.btnShow){
					s += methods.addBtn(_o.url,_o.lastBtn, _o.pageCount);
				}
				return s;
			}
