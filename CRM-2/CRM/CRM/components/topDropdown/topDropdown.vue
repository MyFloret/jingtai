<template>
	<view>
		<view class="topSxRow srarchput">
			<!-- <view class="topSearchView" @click="searchShow = true">
				<u-icon name="search" size="36" color="#666666"></u-icon>
			</view> -->
			<input type="text" @click="searchShow = true" placeholder="搜索" />
		...
		<u-popup v-model="popupShow" width="568rpx" mode="right" border-radius="14" :safe-area-inset-bottom="true"
			...
			<view class="popupCard">
				...
				<view v-else class="kehuRow" @click="selectKhShowFun">已选<text
						class="greenColor">{{selectKhList.length}}</text>条数据</view>
			</view>
		...
		<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
			<view class="searchBox">
				...
				<u-field ref="inputBox" focus  v-if="searchLabel1" v-model="searchValue1" :label="searchLabel1" :placeholder="searchPh1"
		...
		<u-popup v-model="khPopupShow" width="568rpx" mode="right" border-radius="14" :safe-area-inset-bottom="true"
			...
			<view class="popupCard">
				...
				<u-search placeholder="请输入客户名称搜索" v-model="keyword" :animation="true" @search="searchKhBtnFun"
					@custom="searchKhBtnFun"></u-search>
			</view>
			<scroll-view scroll-y="true" class="khScrollView" @scrolltolower="searchKhFun">
				<u-checkbox-group @change="checkboxGroupChange" :wrap="true" class="checkBoxGroup">
					<u-checkbox class="popupCard" v-model="item.checked" v-for="(item, index) in khList" :key="item._id"
						:name="item._id">{{item.clientName}}</u-checkbox>
				</u-checkbox-group>
			</scroll-view>
			<view class="popupBottomBtn">
...
<style>
	...
	.closexiaxiang::before {
		position: absolute;
		content: "";
		top: 50%;
		transform: translateY(-50%);
		right: 34rpx;
		background: url("../../static/img/downloadx.png");
...
/* 	.kongbai::before {
		...
		transform: translateX(-50%);
		transform: translateY(-50%);
		background: url("../../static/img/shaixuan.png");
		background-size: 100%;
		background-repeat: no-repeat;
	} */
</style>