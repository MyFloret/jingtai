...
<template>
	<view class="content">
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false" style="height: calc(100% - 55px);" @scrolltolower="scrollToBottomFun">
			<!--客户基本信息-->
			<shijiHK :item="infoObj" :isDetail="true"></shijiHK>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<hkdDetail :infoObj="infoObj" :isQywx="isQywx"></hkdDetail>
				</view>
				<!--跟进记录-->
				...
				<view v-if="tabIndex == 2">
					<view>
						<czjl :infoObj="infoObj" :dataId="infoObj._id" fl="回款单" pageType="hkd"></czjl>
					</view>
				</view>
				<!--加载更多-->
...
<script>
	let that = '';
	import { crmGenJinApi, crmLianXiRenApi, crmShangJiApi, crmBaoJiaDanApi, crmHeTongApi, crmHkdApi } from '../../../static/utils/api.js'
	import bottomBox from '@/components/bottomBox/bottomBox'
	import shijiHK from '@/components/card/shijiHK.vue'
	import cp1Card from '@/components/card/cp1Card.vue'
	import hkdDetail from '@/components/detail/hkdDetail.vue'
	...
	export default {
		...
		onBackPress(e) {
			if(e.from == 'backbutton' && this.cardIndex != -1) {
				...
				uni.$emit('updateHkdListByIndex', { index: this.cardIndex, obj})
			}
		},
		methods: {
			tabChange(index) {
			...
			pageGenjinFun: function() {
				...
				uni.navigateTo({
					url: '../genJin/add?clientName=' + that.infoObj.clientName + '&clientId=' + that.infoObj.clientId + '&orderType=回款单' + '&glOrderId=' + that.infoObj._id + '&glOrder=' + that.infoObj.hkdCode + '&isQywx=' + that.isQywx + '&external_userid=' + external_userid
				})
			},
			// 修改信息
