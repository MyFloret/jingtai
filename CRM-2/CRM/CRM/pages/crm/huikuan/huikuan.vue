<template>
	<view>
		<topDropdown :isSxShow="true" :isPaiXu="true" :dropdown1="dropdown1" :options1="options1" :optionsSx="sxList" :optionsPx="optionsPx" :isDuoXuan="true" searchLabel1="回款编号" searchPh1="请输入回款单编号" @searchBoxEmitFun="searchBoxEmitFun" @dropDownFun1="dropDownFun1" @optionSxFun="optionSxFun" @optionPxFun="optionPxFun" :isKhShow="true"></topDropdown>
		...
		<scroll-view v-else scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectDataFun"
			...
			<view v-for="(item, index) in list" :key="index" @click="clickFun(item, index)">
				<shijiHK :item="item" :isSelect="isSelect" :index="index" :isQywx="isQywx"></shijiHK>
			</view>
			<getMore :isMore="isMore"></getMore>
		</scroll-view>
		<addBtn v-if="!isQywx" url="./addHk"></addBtn>
		<addBtn v-else url="./addHk?isQywx=true"></addBtn>
	</view>
</template>

<script>
	let that = '';
	import { tyDropDown, dropDownByUser, qywxTyDropDown, qywxDropDownByUser, gjDatePxData, hkdSxDropDownData } from '../../../static/utils/dropdown.js'
	...
	export default {
		...
		onLoad(e) {
			that = this;
			let sxList = JSON.stringify(hkdSxDropDownData);
			...
			uni.$on('updateHkdListByIndex', that.updateHkdListByIndex)
			uni.$on('addHkdItemInListFun', that.addHkdItemInListFun);
		},
		onBackPress() {
			uni.$off('deleteCardFun', that.deleteCardFun);
			uni.$off('updateHkdListByIndex', that.updateHkdListByIndex)
			uni.$off('addHkdItemInListFun', that.addHkdItemInListFun);
		},
		methods: {
			...
			// add页面新增数据
			addHkdItemInListFun: function(e) {
				that.cxGetDataFun()
			},
			// 通过下标更新list数据
			updateHkdListByIndex: function(e) {
