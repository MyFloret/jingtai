<template>
	<view class="schemaList mt32">
		<view class="formTitle">
			...
			<textarea v-else v-model="title" :auto-height="true" placeholder="请在此输入单选框标题" />
		</view>
		<view v-if="!isEdit">
			<radio-group @change="radioChange">
				<label class="flexRow mt32" v-for="(item, index) in options" :key="item.value">
					<view>
						<radio class="formRadio" :value="item.value" :checked="index === formCurrent" />
					</view>
					<view>{{item.text}}</view>
				</label>
			</radio-group>
...
<script>
	...
	export default {
		...
		created() {
			...
			that.formCurrent = obj.formCurrent;
		...
		methods: {
			...
			radioChange: function(evt) {
				for (let i = 0; i < that.options.length; i++) {
					if (that.options[i].value === evt.target.value) {
						that.formCurrent = i;
						break;
					}
				}
