	...
	<view>
		<view class="search_bar">
			<view class="search">
				...
				<input placeholder-style="color:#999; font-size:24rpx;" v-model="searchBarTitle" type="text"
					placeholder="请输入">
		...
		<view class="compontents">
			<view class="common">
				...
				<view class="common_list">
					<view class="listType" v-for="(item, index) in searRecord" :key="index"
						@click="searchModelItemLv(item)">
						{{item.searchName}}
					</view>
				</view>
			</view>
		...
		<view class="compontents bottom">
			<view class="common">
				...
				<view class="common_list">
					<view class="listType" v-for="(items, indexs) in listColorCont" :key="indexs"
						@click="searchModelItem(items)">
						{{items.searchName}}
					</view>
				</view>
			</view>
		...
		methods: {
			...
			searchModel() {
				if (this.searchBarTitle) {
					uni.navigateTo({
						url: "/pages/shareBar/shareBar?content=" + this.searchBarTitle + '&country=' + this
							.selected
					})
			...
			searchModelItem(item) {
				...
				uni.redirectTo({
					url: "/pages/shareBar/shareBar?content=" + item.searchName + '&country=' + this.selected +
						'&id=' + item.id
				})
			},
			searchModelItemLv(e) {
				console.log('item', e);
				uni.redirectTo({
					url: "/pages/shareBar/shareBar?content=" + e.searchName + '&country=' + this.selected +
						'&id=' + e.id
				})
			},
			// 获取input值
